...
interface Props {
  ...
  syllabusEditSlot?: React.ReactNode;
}
    ...
    <section id="workshop" className="py-20 bg-warm-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="bg-white rounded-3xl shadow-lg p-8 md:p-12 mb-12">
          <div className="max-w-3xl mx-auto text-center">
            {content.introParagraphs.map((p, idx) => (
              <p key={idx} className={`${idx === 0 ? 'text-xl' : 'text-lg'} ${idx === 0 ? 'text-gray-600' : 'text-gray-500'} leading-relaxed mb-6`}>
                {p}
              </p>
            ))}
        ...
        <div className="grid md:grid-cols-2 gap-8 mb-12">
          ...
          <div className="bg-blush-100 rounded-3xl p-8">
            ...
            {content.approachParagraphs.map((p, idx) => (
              <p key={idx} className={`${idx === content.approachParagraphs.length - 1 ? 'text-sage-700 font-medium' : 'text-gray-600 leading-relaxed mb-4'}`}>
                {p}
              </p>
            ))}
          </div>
        ...
        <div className="relative bg-white rounded-3xl shadow-lg p-8 md:p-12 mb-12">
          {syllabusEditSlot}
          <h3 className="text-2xl font-bold text-sage-800 mb-8 text-center">{content.syllabusTitle}</h3>
